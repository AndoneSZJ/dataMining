 ...
 * date     2018/6/10 下午2:27
 */
public class OperateInfo {
    public static void main(String[] args) {
        SparkConf sparkConf = new SparkConf().setAppName(OperateInfo.class.getSimpleName());
        String endString = null;
        if (args == null || args.length == 0) {
            sparkConf.setMaster("local[*]");
            endString = "2018-06-11 00:00:00.0";
        } else {
            endString = args[0];
        }
        ...
        //根据网点编号获取城市
        String netPath = "hdfs://vm-xaj-bigdata-da-d01:8020/yst/sta_vem/vem_nettype/*";
        //"hdfs://vm-xaj-bigdata-da-d01:8020/yst/vem/info/net/*";
        ...
        List<Tuple2<String, String>>  initialData2 = sc.textFile(path2).filter(new Function<String, Boolean>() {
...
//                Map<String, String> macMap = bv.getValue();
                ...
                String dt = ss[3];//上下线时间
//                String str [] = macMap.get(vmid).split(",");

//                String createTime = str[6];//创建时间
...
//                String firstCreateTime = null;//最早创建时间
//                double createTimeNum = 0.0;//创建天数
                String firstDt = null;
                long offTime = 0L;
                long onTime = 0L;
                long lastTime = 0L;
                ...
                for (String s : set) {
                    ...
                    LastSta = fs[1];
//                    if(firstCreateTime == null || "".equals(firstCreateTime)){
//                        firstCreateTime = fs[2];
//                    }
//                    if(fs[2] != null && !"".equals(fs[2]) && sdf.parse(firstCreateTime).getTime() > sdf.parse(fs[2]).getTime()){
//                        firstCreateTime = fs[2];
//                    }
                }
...
//                createTimeNum = (double) (end - sdf.parse(firstCreateTime).getTime()) / 1000.0 / 3600.0 / 24.0;

                // 首次上线时间,运营天数,离线时长,在线时长,日均离线次数
                // 近7日日离线次数,7日日离线次数环比//,运营商,网点,点位,机器数,型号,投放时间,七日离线天数,首次投放时间,投放天数
                String resultString = firstDt + "," + difDay + "," + avoffTime + "," + avonTime + "," + avoffCount + ","
                        + avoffCount7 + "," + offCount7on7 + "," + timeNumber;//macMap.get(t._1) + "," + timeNumber+","+firstCreateTime+","+createTimeNum+",1";
                return new Tuple2<String, String>(t._1, resultString);
            }
        }).collect();

        Map<String,String> logMap = new HashMap<String, String>();
        for(Tuple2<String,String> t : initialData2){
            logMap.put(t._1,t._2);
        }

        System.out.println(logMap);
        final Broadcast<Map<String,String>> logBv = sc.broadcast(logMap);

        String path = "hdfs://vm-xaj-bigdata-da-d01:8020/yst/sta_vem/vem_machine/*";
        Logger.getRootLogger().setLevel(Level.OFF);
        JavaPairRDD<String, String> machineRdd = sc.textFile(path).filter(new Function<String, Boolean>() {
            ...
            public Boolean call(String s) throws Exception {
                ...
                return "2".equals(ss[21]) && "1".equals(ss[4]) && "0".equals(ss[5]) && !"620".equals(ss[18])
                        && !"101".equals(ss[18]) && !"7667".equals(ss[18]) && !"".equals(ss[18]) && !"".equals(ss[19]);
            }
        }).mapToPair(new PairFunction<String, String, String>() {
            @Override
            public Tuple2<String, String> call(String s) throws Exception {
                ...
                double day = (double)(end - sdf.parse(createTime).getTime()) / 1000.0 / 3600.0 / 24.0;
                String isFlag = "".equals(ss[23]) ? "0" : "1" ;//判断是否是主机
                return new Tuple2<String, String>(id, yys + "," + wdid + "," + dwid + "," + city + "," + macCount + "," + model+","+createTime+","+day+","+isFlag);
            }
        }).cache();
        ...
        //获取城市登记网点和点位信息
        List<Tuple2<String, String>> numList = machineRdd.filter(new Function<Tuple2<String, String>, Boolean>() {
            @Override
            public Boolean call(Tuple2<String, String> stringStringTuple2) throws Exception {
                String line[] = stringStringTuple2._2.split(",");
                return !"".equals(line[1]) && !"".equals(line[2]) && !"null".equals(line[3]);
            }
        }).mapToPair(new PairFunction<Tuple2<String, String>, String, String>() {
            @Override
            public Tuple2<String, String> call(Tuple2<String, String> stringStringTuple2) throws Exception {
                String line[] = stringStringTuple2._2.split(",");
                String city = line[3];//城市
                String netId = line[1];//网点
                String pointId = line[2];//点位
                String isFlag = line[8];//是否是主机
                return new Tuple2<String, String>(city, netId + "," + pointId+","+isFlag);
            }
        }).reduceByKey(new Function2<String, String, String>() {
            @Override
            ...
            public Tuple2<String, String> call(Tuple2<String, String> stringStringTuple2) throws Exception {
                ...
                Map<String, Integer> netMap = new HashMap<String, Integer>();
                Map<String, Integer> pointMap = new HashMap<String, Integer>();

                for (String s : lines) {
                    String[] line = s.split(",");
                    if("1".equals(line[2])){
                        if (!netMap.containsKey(line[0])) {
                            netMap.put(line[0], 1);
                        }
                        if (!pointMap.containsKey(line[1])) {
                            pointMap.put(line[1], 1);
                        }
                    }
                }
                //城市名称,网点数,点位数
                return new Tuple2<String, String>(stringStringTuple2._1, netMap.size() + "," + pointMap.size());
            }
        }).collect();

        Map<String, String> numMap = new HashMap<String, String>();
        for (Tuple2<String, String> t : numList) {
            numMap.put(t._1, t._2);
        }
        System.out.println(numMap);

        final Broadcast<Map<String, String>> bvNum = sc.broadcast(numMap);


        JavaRDD<String> machineData = machineRdd.mapToPair(new PairFunction<Tuple2<String, String>, String, String>() {
            @Override
            public Tuple2<String, String> call(Tuple2<String, String> stringStringTuple2) throws Exception {
                Map<String,String> logMachineMap = logBv.getValue();
                String s = ",,,,,,,";
                String log = logMachineMap.get(stringStringTuple2._1);
                int num = 0;//判断机器是否运营
                if (log != null) {
                    s = log;
                    num = 1;
                }
                return new Tuple2<String, String>(stringStringTuple2._1,s+","+stringStringTuple2._2+","+num);
            }
            ...
            public String call(Tuple2<String, String> stringStringTuple2) throws Exception {
                ...
                // 首次上线时间,运营天数,离线时长,在线时长,日均离线次数
                // 近7日日离线次数,7日日离线次数环比,七日离线天数,运营商,网点,点位,城市,机器数,型号,首次投放时间,投放天数,是否是主机,是否运营
        ...
        List<Tuple2<String,String>> operate = machineData.mapToPair(new PairFunction<String, String, String>() {
            @Override
            public Tuple2<String, String> call(String s) throws Exception {
                String line[] = s.split(",");
                String netId = line[10];//网点
                String pointId = line[11];//点位
                String city = line[12];//城市
                String isFlag = line[17];//是否是主机
                String isOperate = line[18];//是否运营

                return new Tuple2<String, String>(city,netId+","+pointId+","+isFlag+","+isOperate);
            }
        }).reduceByKey(new Function2<String, String, String>() {
            @Override
            public String call(String s, String s2) throws Exception {
            ...
            public Tuple2<String, String> call(Tuple2<String, String> stringStringTuple2) throws Exception {
                ...
                for (String s : lines) {
                    String[] line = s.split(",");
                    if("1".equals(line[2]) && "1".equals(line[3])){//主机,在运营
                        if (!netMap.containsKey(line[0])) {
                            netMap.put(line[0], 1);
                        }
                        if (!pointMap.containsKey(line[1])) {
        ...
        for(Tuple2<String,String> tuple2 : operate){
            operateMap.put(tuple2._1,tuple2._2);
        }

        final Broadcast<Map<String, String>> operateMapBv = sc.broadcast(operateMap);

        JavaPairRDD<String, String> pointOperateStat = getOperateStat(machineData, sc, "P", bvNum,operateMapBv).cache();
        JavaPairRDD<String, String> netOperateStat = getOperateStat(machineData, sc, "N", bvNum,operateMapBv).cache();
        JavaPairRDD<String, String> operatorOperateStat = getOperateStat(machineData, sc, "O", bvNum,operateMapBv).cache();
        JavaPairRDD<String, String> allOperateStat = getOperateStat(machineData, sc, "A", bvNum,operateMapBv).cache();
        JavaPairRDD<String, String> cityOperateStat = getOperateStat(machineData, sc, "C", bvNum,operateMapBv).cache();
...
//
    ...
    private static JavaPairRDD<String, String> getOperateStat(JavaRDD<String> rdd, JavaSparkContext sc, String type, Broadcast<Map<String, String>> numBv, Broadcast<Map<String, String>> operateMapBv) {
        ...
        JavaPairRDD<String, String> result = rdd.mapToPair(new PairFunction<String, String, String>() {
            ...
            @Override
            public Tuple2<String, String> call(String s) throws Exception {
                // 首次上线时间,运营天数,离线时长,在线时长,日均离线次数 1-5
                // 近7日日离线次数,7日日离线次数环比,七日离线天数, 6-8
                // 运营商,网点,点位,城市,机器数,型号,9-14
                // 首次投放时间,投放天数,是否是主机,是否运营 15-18
                String type = bv.getValue();
                String[] ss = s.split(",");
                String firstDt = ss[1];//首次上线时间
                String difDay = ss[2];//运营天数
                String avoffTime = ss[3];//离线时长
                String offCount = ss[5];//日均离线次数
                String offCount7 = ss[6];//近7日日离线次数
                String yys = ss[9];//运营商
                String wd = ss[10];//网点
                String dw = ss[11];//点位
                String city = ss[12];//城市
                String timeNumber = "".equals(ss[8]) ? "0" : ss[8];//七日离线天数
                String createTime = ss[15];//首次投放时间
                String createNum = ss[16];//投放天数
                String isFlag = ss[17];//是否是主机
                String isOperate = ss[18];//是否运营
                String key = "1";
                if ("P".equals(type)) {
                    key = dw;
                ...
                String macCount = ss[13];//机器数
//				System.out.println(firstDt + "," + difDay + "," + avoffTime + "," + offCount + "," + offCount7 + "," + macCount+","+dw +"," + timeNumber);
                return new Tuple2<String, String>(key,
                        firstDt + "," + difDay + "," + avoffTime + "," + offCount + "," + offCount7 + "," + macCount + "," + dw + "," + timeNumber+","+createTime+","+createNum+","+isOperate+","+isFlag) ;
            }
        }).reduceByKey(new Function2<String, String, String>() {
            ...
            public Tuple2<String, String> call(Tuple2<String, String> t) throws Exception {
                ...
                for (String s : ss) {
                    ...
                    if(!"".equals(fs[9]) && (Double.parseDouble(fs[9]) > createNum)){
                        createNum = Double.parseDouble(fs[9]);
                        createTime = fs[8];
                    }
                    if("1".equals(fs[11])){//判断是否是主机,是主机则存在是否运营
                        if("0".equals(fs[10])){//未运营
                            macNumber += 1 ;
                        }else {//运营
                            operateMacNumber += 1 ;
                        }
                ...
                // 最早运营时间,运营天数,主机数量,机器数量,日均掉线5次以上的主机数,  1-5
                // 近7日日均掉线5次以上的主机数,近7日日均掉线20次以上的主机数,日均掉线时长超过6小时的主机数, 6-8
                // 日均掉线超过10次的主机数,点位数,七日离线天数,最早投放时间,投放天数,运营网点数,运营点位数,运营主机数,未运营主机数,投放网点数,投放点位数 9-19
                String result = firstDay + "," + diffDays + "," + mstMacCount + "," + macCount + "," + offenOffMacCount
                        ...
                        + longTimeOffMacCount2 + "," + map.size() + "," + timeNunber+","+createTime+","+createNum;


                //运营网点数,点位数
                if ("C".equals(bv.getValue())) {
                    Map<String, String> numMap = operateMapBv.getValue();
                    String num = numMap.get(t._1);
                    result += "," + num;
                }else if ("A".equals(bv.getValue())) {
                    Map<String, String> numMap = operateMapBv.getValue();
                    Iterator<Map.Entry<String, String>> entries = numMap.entrySet().iterator();
                    Integer pointNum = 0;
                    Integer netNum = 0;
                    while (entries.hasNext()) {
                        ...
                        netNum += Integer.parseInt(line[0]);
                        pointNum += Integer.parseInt(line[1]);
                    }
                    result += ("," + netNum + "," + pointNum);
                }else{
                    result += ",1,1";
                }

               
                result += (","+operateMacNumber+","+macNumber);

                //投放网点数,点位数
                if ("C".equals(bv.getValue())) {
                    Map<String, String> numMap = numBv.getValue();
                    String num = numMap.get(t._1);
                    result += "," + num;
                }else if ("A".equals(bv.getValue())) {
                    Map<String, String> numMap = numBv.getValue();
