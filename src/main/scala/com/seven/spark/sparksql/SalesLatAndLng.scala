...
object SalesLatAndLng {
  ...
  def main(args: Array[String]): Unit = {
    val netData = spark.read.option("delimiter", ",").csv("/yst/seven/data/aaaaa/*")
    ...
    spark.read.option("delimiter", ",").option("header", "true").csv("/yst/seven/data/netData/*").createOrReplaceTempView("net")
    ...
    data.repartition(1).write.mode(SaveMode.Overwrite).option("header", "true").csv("/yst/seven/data/bbbb/cccc/")
  }
}
