    ...
    val order = sc.textFile(orderPath)
      ...
      .mapPartitions(x => {
        ...
        x.foreach(row =>{
          ...
          val date = new Date()
          val pointByBetMap = pointBv.value//点位与网点之间的关系
          var pointMap:Map[String,Int] = Map()
          val long = 1000 * 3600 * 24 * 20L//20天时间
          ...
          for(line <- lines){
            ...
            if(df.parse(l(6)).getTime > (date.getTime - long)){
              moneys += l(4).toDouble//20日销售金额
              val m = l(4).toDouble
              if(!pointMap.contains(l(9))){//20日内网点有销售记录的点位
                pointMap += (l(9) -> 0)
              }
            }
          }
        ...
        .mapPartitions(x => {
          ...
          x.foreach(row =>{
            ...
            val city = row._2.split(",")(1)//城市
            val netId = row._1.split(",")(0)//网点
            val coefficient = coefficientOfVariation(row._2.split(",")(2))//变异系数
            val money = row._1.split(",")(1)//销售额
