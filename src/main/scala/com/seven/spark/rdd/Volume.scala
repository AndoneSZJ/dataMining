...
object Volume {
  ...
  def salesOperate(operatePath:String,orderPath:String,writePath:String): Unit ={
    ...
    def sale7(time:String,num:String,n:String): String ={
      var nn = 3.0//机器数，如果为空则初始值为3
      var number = 0.0//销量为空，则为0
      var timeOut = 0.0
      if(!"".equals(num)){
        number = num.toDouble
      }
      ...
      if("".equals(time) || timeOut > 7){//运营时间小于七天则取实际时间
        number / 7 / nn + ""
      }else{
        number / timeOut / nn + ""
      }
    }
    spark.udf.register("sale7",sale7 _)//注册
    ...
    def sale30(time:String,num:String,n:String): String ={
      ...
      if("".equals(time) || timeOut > 30){//如果运营时间小于30天，则取实际时间
        number / 30 / nn + ""
      }else{
        number / timeOut / nn + ""
      }
