...
object SalesGoodPoint {
  ...
  def getOrderPointData(path:String): RDD[(String,Double)] ={
    ...
    val date = new Date()
    val long = 1000 * 3600 * 24 * 15L//15天时间搓
  ...
  def getPointByMoneyThan60(rdd:RDD[(String,Double)],broadcast: Broadcast[util.HashMap[String,String]]): Unit ={
    ...
    val data = rdd.filter(x =>{//过滤运营时间小于7天的点位
      ...
      val hashMap = new mutable.HashMap[String,Double]()
        x.foreach(row =>{
          val map = broadcast.value//获取广播map
          val pointId = row._1.split("_")(0)
          val time = map.get(pointId).toDouble//根据点位id获取运营时间
          var avgMoney = 0.0
        ...
        hashMap.iterator
      }).filter(x => x._2 >= 60).mapPartitions(x =>{//过滤日均销售额小于60的点位。输出网点id
  ...
  def salesNullPoint(path:String,rdd:RDD[(String,Double)]): Unit ={
    ...
    val data = sc.textFile(path).filter(x => {//获取所有点位信息
      val line = x.split(",")
