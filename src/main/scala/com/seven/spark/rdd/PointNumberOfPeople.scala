...
object PointNumberOfPeople {
  ...
  def communityToMap(row:String): (String,String) ={
    ...
    val communityId = line(0)//小区id
    val communityName = line(1)//小区名称
  ...
  def nettypeToMap(row:String): (String,String) ={
    val line = row.toString.split(",")
    val nettypeId = line(0)//网点id
    val createtime = line(4)//创建时间
    val name = line(1)//名称
  ...
  def onePointMap(row:String,nettypeBv:Broadcast[util.HashMap[String, String]]): (String,String) ={
    ...
    val ctrateTime = net.split("&")(0)//创建时间
    val communityName = point.split("&")(2)//小区名称
  ...
  def twoPointMap(row_1:String,row_2:String): (String,String) ={
    val format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
    val date = new Date()
    val long = 1000 * 3600 * 24 * 7L//七天时间
    val lines = row_2.split("&")
    val ll = lines(0)
    val city = ll.split(",")(4)//城市
    val createTime = ll.split(",")(6)//创建时间
    val communityName = ll.split(",")(3)//小区名称
    var time = format.parse(createTime).getTime//获取创建时间搓
    var map:Map[String,Long] = Map()
    for(l <- lines){
      val line = l.split(",")
      if(!map.contains(line(1))){//去重，多条记录只算一次
    ...
    while (time < date.getTime){
      ...
      for(k <- map.keySet){
        if(map(k) < timeNum && map(k) > time){//获取在七天内的销量
          num += 1
        }
      }
      number += num+","
