...
object SalesNetAvgMount {
  ...
  def getOrderDataNet(path:String,operateBv: Broadcast[util.HashMap[String,String]],basicNetBv: Broadcast[util.HashMap[String,String]]): Unit ={
    ...
    val order = sc.textFile(path).filter(x => {//选取7天内的销售数据
      val line = x.split(",")
      val time = line(0)
      ...
      var list = List[String]()
        x.foreach(row =>{
          ...
          if(net == null){
            ...
            val basic = net.split(",")
            val netName = basic(1)//网点名称
            val householdTotalNum = basic(25)//小区户数
            val householdCheckInNum = basic(16)//入住户数
            val propertyCosts = basic(22)//物业费
